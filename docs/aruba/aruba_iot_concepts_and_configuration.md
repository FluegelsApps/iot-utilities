...
|`zigbee-channel <channel>`|`zigbee-channel <channel>`|Channel to be used.<br> This parameter is available only when `radio-mode` is set to ***zigbee*** or ***"ble zigbee"***.<br> Available options are:<br> **auto** - Selects the channel automatically (default) <br> Value range: ***11 ... 26*** - Specify the channel manually|
